...
`ember-move-uci-mark` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies uci behavior through log and snapshot fixtures, with replay consistency checks and local-only command execution.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 3.
