...
ember-move-uci-mark is a Elixir project for chess and game engines. It focuses on this technical goal: Build an Elixir toolkit that studies uci behavior through log and snapshot fixtures, with replay consistency checks and local-only command execution.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 150, risk penalty 4, latency penalty 2, and weight bonus 3. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
